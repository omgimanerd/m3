...
class CurseForgeWrapper:  # pylint: disable=too-few-public-methods
    ...
    def _request(self, path: Path, body: dict = None) -> dict:
        try:
            url = urljoin(CF_BASE_URL, Path(CF_API_VERSION) / str(path))
    ...
    def _unpack_request(self, path: Path, unpacker: Callable[[object], object],
                        body: dict = None) -> object:
        try:
            json = self._request(path, body=body)
